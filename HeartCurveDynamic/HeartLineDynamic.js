...
function nightly() {
    ...
    if(icount==50) {
        while (icount <= 1000) {
            document.getElementById("love-func").value = "(0.64*sqrt(abs(x))-0.8+1.2^abs(x)*cos(" + icount + "*x))*sqrt(cos(x))";
            icount++;
            change();
        }
        icount = 50;
...
function Calc(fun, X, Value) {
    ...
    for (var i = 0; i < fun.length; i++) {
        ...
        if (j == X.length)
            ...
            } else if (fun[i] == '.') base = 1;
            else if (fun[i] == '+' && (i == 0 || priority(fun[i - 1]) != -1)) ;
            else if (fun[i] == '-' && (i == 0 || priority(fun[i - 1]) != -1)) sign = -1;
            else if (fun[i] == 'e')
                if (i == 0 || isOpt(fun[i - 1])) now = Math.E;
                else now *= Math.E;
            else if (fun[i] == 'p' && fun[i + 1] == 'i') {
                if (i == 0 || isOpt(fun[i - 1])) now = Math.PI;
                else now *= Math.PI;
                i += 1;
            } else if (isDigit(fun[i]))
                if (base == 0) now = now * 10 + (fun[i] - '0');
                else base /= 10, now += base * (fun[i] - '0');
            else if (isChar(fun[i])) f += fun[i];
            else if (isOpt(fun[i])) {
                number.push(now * sign);
                now = 0;
                sign = 1;
